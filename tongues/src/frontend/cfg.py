...
@dataclass
class ResolveCtx:
    """Lightweight context for the flow resolver."""
...
def resolve_types(
    ...
    ctx: ResolveCtx,
) -> dict[int, dict[str, TypeNode]]:
    """Resolve types at each flow node via worklist forward dataflow.

    assigned_types maps FlowAssign node ids to their inferred types.
    """
    types_at: dict[int, dict[str, TypeNode]] = {}
    start_env: dict[str, TypeNode] = {}
    ikeys = list(initial_types.keys())
    k = 0
    while k < len(ikeys):
        start_env[ikeys[k]] = initial_types[ikeys[k]]
        k += 1
    types_at[graph.start_id] = start_env
    worklist: list[int] = [graph.start_id]
    ...
    while len(worklist) > 0:
        ...
        out = _compute_out(node, in_types, assigned_types, types_at, ctx, graph)
        old = types_at.get(nid)
        changed = False
        if old is None:
            changed = True
        else:
            okeys = list(out.keys())
            if len(okeys) != len(old):
                changed = True
            else:
                ci = 0
                while ci < len(okeys):
                    ok = okeys[ci]
                    oval = old.get(ok)
                    if oval is None or not type_eq(oval, out[ok]):
                        changed = True
                    ci += 1
        if changed:
            if isinstance(node, FlowLoopHead):
                count: int = 0
                got = iterations.get(nid)
                if got is not None:
                    count = got
                count += 1
                iterations[nid] = count
                if count > max_iter:
                    continue
            types_at[nid] = out
...
def _merge_predecessors(
    prev_ids: list[int], types_at: dict[int, dict[str, TypeNode]]
) -> dict[str, TypeNode]:
    """Merge type environments from predecessor nodes."""
    if len(prev_ids) == 0:
        return {}
    if len(prev_ids) == 1:
        env = types_at.get(prev_ids[0])
        if env is None:
            return {}
        result: dict[str, TypeNode] = {}
        ekeys = list(env.keys())
        ei = 0
        while ei < len(ekeys):
            result[ekeys[ei]] = env[ekeys[ei]]
            ei += 1
        return result
    all_keys: list[str] = []
    pi = 0
    while pi < len(prev_ids):
        env = types_at.get(prev_ids[pi])
        if env is not None:
            ekeys = list(env.keys())
            ei = 0
            while ei < len(ekeys):
    ...
    while ki < len(all_keys):
        ...
        if len(variants) == 1:
            ...
            result[k] = combine_types(variants)
        ki += 1
    return result
...
def _compute_out(
    ...
    oi = 0
    while oi < len(ikeys):
        out[ikeys[oi]] = in_types[ikeys[oi]]
        oi += 1
    if isinstance(node, FlowStart):
        return out
    if isinstance(node, FlowAssign):
        atype = assigned_types.get(node.id)
