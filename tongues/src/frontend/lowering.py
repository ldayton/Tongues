...
def _typenode_to_ttype(pos: Pos, t: TypeNode) -> TType:
    ...
    if isinstance(t, TupleType):
        ...
        while i < len(t.elements):
            parts.append(_typenode_to_ttype(pos, t.elements[i]))
            i += 1
        if len(parts) >= 2:
    ...
    if isinstance(t, FuncType):
        ...
        while i < len(t.params):
            fn_parts.append(_typenode_to_ttype(pos, t.params[i]))
            i += 1
    ...
    if isinstance(t, UnionType):
        ...
        while i < len(t.variants):
            parts2.append(_typenode_to_ttype(pos, t.variants[i]))
            i += 1
        if len(parts2) >= 2:
...
def _make_named_call(
    ...
    while i < len(pos_args):
        targs.append(TArg(pos, None, pos_args[i]))
        i += 1
    i = 0
    while i < len(named):
...
def _lower_name_call(
    ...
    if fname == "min" or fname == "max":
        ...
        if len(lowered) >= 3:
            ...
            j = 2
            while j < len(lowered):
                result = _make_call(pos, builtin, [result, lowered[j]])
                j += 1
            return result
        return _make_call(pos, builtin, lowered)
    if fname == "pow":
    ...
    if fname == "int":
        ...
        if len(args) >= 1 and isinstance(args[0], dict):
            ...
            return _make_call(
                pos, "ParseInt", [arg, TIntLit(pos, 10, "10", {})]
            )
    if fname == "float":
        if len(args) == 0:
            return TFloatLit(pos, 0.0, "0.0", {})
    ...
    if fname == "bool":
        ...
        if len(args) > 0 and isinstance(args[0], dict):
            ...
            if _is_type_dict(arg_type, ["int"]):
                return TBinaryOp(
                    pos, "!=", arg, TIntLit(pos, 0, "0", {}), {}
                )
            if _is_type_dict(arg_type, ["float"]):
                return TBinaryOp(
                    pos, "!=", arg, TFloatLit(pos, 0.0, "0.0", {}), {}
                )
            if _is_type_dict(arg_type, ["string"]):
                return TBinaryOp(
                    pos, "!=", arg, TStringLit(pos, "", {}), {}
                )
            if _is_type_dict(arg_type, ["bool"]):
                return arg
            if isinstance(arg_type, TupleType):
                ...
                if _is_ast(args[0], "Constant"):
                    ...
                    if isinstance(s_jv, JStr):
                        ...
                        while ci < len(s_jv.value):
                            elems.append(
                                TStringLit(pos, s_jv.value[ci : ci + 1], {})
                            )
                            ci += 1
...
def _lower_method_call(
    ...
    if len(keywords) > 0:
        ...
        while i < len(args):
            ...
            i += 1
    return TCall(
        pos, TFieldAccess(pos, obj, method_name, {}), lowered_args, {}
    )
...
def _method_side_effects(value_node: ASTNode, env: _Env, ctx: _LowerCtx) -> list[TStmt]:
    ...
    if _is_type_dict(actual, ["Map"]) and method == "setdefault":
        if (
            ...
            cond = TUnaryOp(
                pos, "!", _make_call(pos, "Contains", [obj, key]), {}
            )
            assign = TAssignStmt(
                pos, TIndex(pos, obj, key, {}), default, {}
            )
...
def _lower_any_all(fname: str, node: ASTNode, env: _Env, ctx: _LowerCtx) -> TExpr:
    ...
    let_stmt = TLetStmt(
        pos, rname, bool_type, TBoolLit(pos, default_val, {}), {}
    )
    cond: TExpr = elt_expr
    if not is_any:
...
def _lower_listcomp(node: ASTNode, env: _Env, ctx: _LowerCtx) -> TExpr:
    ...
    let_stmt = TLetStmt(
        pos, rname, list_type, TListLit(pos, [], {}), {}
    )
...
def _expand_listcomp(node: ASTNode, env: _Env, ctx: _LowerCtx) -> list[TStmt]:
    ...
    let_stmt = TLetStmt(
        pos, "__result__", result_type, TListLit(pos, [], {}), {}
    )
...
def _expand_dictcomp(node: ASTNode, env: _Env, ctx: _LowerCtx) -> list[TStmt]:
    ...
    result_type: TType = _typenode_to_ttype(pos, ret_type)
    let_stmt = TLetStmt(
        pos, "__result__", result_type, _make_call(pos, "Map", []), {}
    )
    idx_target = TIndex(pos, result_var, key_expr, {})
    body: list[TStmt] = [TAssignStmt(pos, idx_target, val_expr, {})]
    ifs = get_nodes(gen, "ifs")
...
def _lower_as_bool(node: ASTNode, env: _Env, ctx: _LowerCtx) -> TExpr:
    ...
    if _is_type_dict(expr_type, ["float"]):
        expr = _lower_expr(node, env, ctx)
        return TBinaryOp(
            pos, "!=", expr, TFloatLit(pos, 0.0, "0.0", {}), {}
        )
    ...
    if isinstance(value_jv, JDict):
        ...
        if _is_type_dict(val_type, ["void"]) and isinstance(type_dict, OptionalType):
            ...
            if already_declared:
                ...
                stmts.append(
                    TAssignStmt(pos, target, _make_call(pos, "Set", []), {})
                )
            else:
                stmts.append(
...
def _lower_aug_assign(node: ASTNode, env: _Env, ctx: _LowerCtx) -> list[TStmt]:
    ...
    if op_type == "Add":
        ...
        if _is_type_dict(target_type, ["string", "bytes"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Concat", [target, value]), {}
                )
            ]
        if _is_type_dict(target_type, ["Slice"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Concat", [target, other]), {}
                )
            ]
    ...
    if op_type == "BitOr":
        ...
        if _is_type_dict(target_type, ["Map"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Merge", [target, value]), {}
                )
            ]
        if _is_type_dict(target_type, ["Set"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Union", [target, value]), {}
                )
            ]
    ...
    if op_type == "Mult":
        ...
        if _is_type_dict(target_type, ["Tuple"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Repeat", [target, value]), {}
                )
            ]
    ...
    if op_type == "BitXor":
        ...
        if _is_type_dict(target_type, ["Set"]):
            ...
            return [
                TAssignStmt(
                    pos, target, _make_call(pos, "Difference", [u, inter]), {}
                )
            ]
    target = _lower_expr(target_node, env, ctx)
    value = _lower_expr(value_node, env, ctx)
...
def _lower_isinstance_chain(
    ...
    while i < len(chain):
        ...
        pattern = TPatternType(
            pos, binding_name, TIdentType(pos, type_name), {}
        )
        ...
        if _is_ast(arg_node, "Constant"):
            ...
            if isinstance(s_jv, JStr):
                ...
                while ci < len(s_jv.value):
                    ...
                    ci += 1
                return _make_call(
                    pos, "SetFromList", [TListLit(pos, ens_elems, {})]
                )
        return _make_call(
            pos,
            "SetFromList",
    ...
    if _is_ast(value, "Call"):
        ...
        if _is_ast(func, "Attribute"):
            ...
            if _is_type_dict(obj_type, ["Slice"]) and method == "clear":
                obj = _lower_expr(obj_node, env, ctx)
                return [
                    TAssignStmt(pos, obj, TListLit(pos, [], {}), {})
                ]
            # list.reverse() → xs = Reversed(xs)
            if _is_type_dict(obj_type, ["Slice"]) and method == "reverse":
                ...
                return [
                    TAssignStmt(
                        pos, obj, _make_call(pos, "Reversed", [obj]), {}
                    )
                ]
            # list.sort() → xs = Sorted(xs)
            if _is_type_dict(obj_type, ["Slice"]) and method == "sort":
                obj = _lower_expr(obj_node, env, ctx)
                return [
                    TAssignStmt(pos, obj, _make_call(pos, "Sorted", [obj]), {})
                ]
            ...
            if _is_type_dict(obj_type, ["Map"]) and method == "pop":
                ...
                if len(vargs) > 0 and isinstance(vargs[0], dict):
                    ...
                    return [
                        TExprStmt(
                            pos, _make_call(pos, "Delete", [obj, key]), {}
                        )
                    ]
