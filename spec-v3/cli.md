...
Program entry point. Parses command-line arguments, reads source input, invokes the compilation pipeline, writes output.
...
| Directory    | `tongues ./src`    | Gather .py files, compile as project (see `frontend.md`, Merge) |
...
Three flags control cross-target consistency. Strict flags are stored on the Taytsh Module node. See `taytsh.md`, Math Semantics for full semantics.
