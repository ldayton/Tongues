...
_transpile-tongues target:
    #!/usr/bin/env bash
    set -euo pipefail
    declare -A ext=([python]=py [ruby]=rb [perl]=pl [javascript]=js [java]=java [go]=go [taytsh]=ty)
...
# Compile transpiled Go to binary
_compile-go:
    #!/usr/bin/env bash
    set -euo pipefail
    printf '\033[32m[compile:go]\033[0m\n'
    start=$SECONDS
    if ! go build -o tongues/.out/tongues-go tongues/.out/tongues.go 2>&1; then
        printf '\033[31m[compile:go] %ds (FAILED)\033[0m\n' "$((SECONDS - start))"
        exit 1
    fi
    printf '\033[32m[compile:go] %ds\033[0m\n' "$((SECONDS - start))"

# Run taytsh app tests through the treewalker with Go binary
_treewalker-go:
    #!/usr/bin/env bash
    set -uo pipefail
    printf '\033[32m[tw-taytsh-apptests-go]\033[0m\n'
    start=$SECONDS
    passed=0; failed=0
    cd tongues
    for f in tests/taytsh/app/*.ty; do
        name=$(basename "$f" .ty)
        if .out/tongues-go taytsh "$f" >/dev/null 2>&1; then
            passed=$((passed + 1))
        else
            echo "  FAIL $name"
            failed=$((failed + 1))
    ...
    elapsed=$((SECONDS - start))
    if [ $failed -eq 0 ]; then
        printf '\033[32m[tw-taytsh-apptests-go] %ds (%d passed)\033[0m\n' "$elapsed" "$passed"
    else
        printf '\033[31m[tw-taytsh-apptests-go] %ds (%d passed, %d failed)\033[0m\n' "$elapsed" "$passed" "$failed"
        exit 1
    fi

# Run taytsh app tests through the VM with Go binary
_vm-go:
    #!/usr/bin/env bash
    set -uo pipefail
    printf '\033[32m[vm-taytsh-apptests-go]\033[0m\n'
    start=$SECONDS
    passed=0; failed=0
    cd tongues
    for f in tests/taytsh/app/*.ty; do
        name=$(basename "$f" .ty)
        if .out/tongues-go taytsh --vm "$f" >/dev/null 2>&1; then
            passed=$((passed + 1))
        else
            echo "  FAIL $name"
            failed=$((failed + 1))
    ...
    elapsed=$((SECONDS - start))
    if [ $failed -eq 0 ]; then
        printf '\033[32m[vm-taytsh-apptests-go] %ds (%d passed)\033[0m\n' "$elapsed" "$passed"
    else
        printf '\033[31m[vm-taytsh-apptests-go] %ds (%d passed, %d failed)\033[0m\n' "$elapsed" "$passed" "$failed"
        exit 1
    fi

# Self-transpile to Go, compile, and run tests
lang-go:
    #!/usr/bin/env bash
    set -uo pipefail
    pids=()
    ...
    trap 'cleanup; trap - TERM; kill -TERM $$' TERM
    trap 'cleanup' EXIT
    just -f {{justfile()}} _transpile-tongues go
    just -f {{justfile()}} _compile-go
    failed=0
    declare -A results=()
    just -f {{justfile()}} cross-equivalence go & pids+=($!); pid_names[$!]=cross-equivalence-go
    just -f {{justfile()}} _treewalker-go & pids+=($!); pid_names[$!]=tw-taytsh-apptests-go
    just -f {{justfile()}} _vm-go & pids+=($!); pid_names[$!]=vm-taytsh-apptests-go
    just -f {{justfile()}} _vm-test-tongues-go & pids+=($!); pid_names[$!]=vm-test-tongues-go
    printf '\033[32m[test-tongues-go]\033[0m\n'
    start=$SECONDS
    cd tongues
    .out/tongues-go taytsh --emit go src > /dev/null; rc=$?
    elapsed=$((SECONDS - start))
    if [ $rc -eq 0 ]; then
        printf '\033[32m[test-tongues-go] %ds\033[0m\n' "$elapsed"
        results[test-tongues-go]=PASS
    else
        printf '\033[31m[test-tongues-go] %ds (FAILED)\033[0m\n' "$elapsed"
        results[test-tongues-go]=FAIL
        failed=1
    fi
    for pid in "${pids[@]}"; do
        name="${pid_names[$pid]}"
...
test-tongues target *args:
    ...
    if [ "{{target}}" = "go" ]; then
        just -f {{justfile()}} _test-tongues-go {{args}}
        exit $?
    fi
    declare -A ext=([python]=py [ruby]=rb [perl]=pl [javascript]=js)
    declare -A runner=([python]="uv run python3" [ruby]=ruby [perl]=perl [javascript]=node)
...
# Run test suite with Go binary
_test-tongues-go *args:
    #!/usr/bin/env bash
    set -uo pipefail
    just -f {{justfile()}} _transpile-tongues go
    just -f {{justfile()}} _compile-go
    ...
    start=$SECONDS
    cd tongues
    .out/tongues-go taytsh --emit go src {{args}} > /dev/null; rc=$?
    elapsed=$((SECONDS - start))
    if [ $rc -eq 0 ]; then
        printf '\033[32m[test-tongues-go] %ds\033[0m\n' "$elapsed"
    else
...
# Run test suite through VM with Go binary
_vm-test-tongues-go *args:
    #!/usr/bin/env bash
    set -euo pipefail
    just -f {{justfile()}} _transpile-tongues go
    just -f {{justfile()}} _compile-go
    just -f {{justfile()}} _transpile-tongues taytsh
    printf '\033[32m[vm-test-tongues-go]\033[0m\n'
    start=$SECONDS
    cd tongues
    .out/tongues-go taytsh --vm .out/tongues.ty --emit go src {{args}} > /dev/null; rc=$?
    elapsed=$((SECONDS - start))
    if [ $rc -eq 0 ]; then
        printf '\033[32m[vm-test-tongues-go] %ds\033[0m\n' "$elapsed"
    else
        printf '\033[31m[vm-test-tongues-go] %ds (FAILED)\033[0m\n' "$elapsed"
    fi
    exit $rc
...
cross-equivalence target:
    #!/usr/bin/env bash
    set -euo pipefail
    declare -A ext=([ruby]=rb [perl]=pl [javascript]=js [java]=java [go]=go)
    e=${ext[{{target}}]}
...
_vm-test-tongues target *args:
    ...
    if [ "{{target}}" = "go" ]; then
        just -f {{justfile()}} _vm-test-tongues-go {{args}}
        exit $?
    fi
    declare -A ext=([python]=py [ruby]=rb [perl]=pl [javascript]=js)
    declare -A runner=([python]="uv run python3" [ruby]=ruby [perl]=perl [javascript]=node)
...
full-check:
    ...
    just -f {{justfile()}} lang-go & pids+=($!); labels+=("lang-go")
    failed=0
    for i in "${!pids[@]}"; do
        if ! wait "${pids[$i]}"; then
