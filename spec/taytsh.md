...
Generic type parameters are built-in only (`list[T]`, `map[K, V]`, `set[T]`, `fn[T..., R]`). User-defined structs, interfaces, and functions are monomorphic. Generic Python source is monomorphized during lowering; the IR only represents concrete instantiations.
...
`NilError` has no source-language equivalent. It is generated by the lowerer for `Unwrap` calls — cases where the source language's narrowing rules cannot statically prove non-nil and a runtime check is needed. Source-level `AssertionError` maps to `AssertError` during lowering.
