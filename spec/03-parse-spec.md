...
The tokenizer uses explicit `while i < len(...)` loops (no generators). The parser produces dict-based AST nodes. This module has no external dependencies and no knowledge of the Taytsh type system.
...
## Divergences from ast Module

| Area               | `ast` module                                           | This parser                                                                             |
| ------------------ | ------------------------------------------------------ | --------------------------------------------------------------------------------------- |
...
| `Constant.kind`    | `"u"` for unicode literals                             | Omitted; Python 2 legacy                                                                |
| Module modes       | `Expression`, `Interactive`, `FunctionType`            | Only `Module` mode                                                                      |
...
Source code parsed to dict-based AST. Structure follows `ast.parse()` output (see divergences above). All tokens consumed. Syntax errors reported with line and column.
